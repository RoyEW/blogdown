...
additional <- multiplot("TSLA","HYMTF", "FCAU","RACE","NSANY","F","BMW.MI","BMW.DE","HMC","GM","VOW.DE","TM", period = "max",length = 30) %>% 
  filter(!is.na(price),
         price>1) %>%
  group_by(ticker) %>% 
  mutate(percentage_price = ((price/lag(price,1)) - 1)) %>%
...
data %>% 
   ggplot()+
   geom_rect(aes(fill=market_state,xmin=date-5,xmax=date+5,ymin=ifelse(market_state=="+",0,data$percentage_price),ymax=ifelse(market_state=="+",data$percentage_price,0)),alpha=0.03)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
...
tsla <- multiplot("TSLA","HYMTF", "FCAU","RACE","NSANY","F","BMW.MI","BMW.DE","HMC","GM","VOW.DE","TM", period = "max",length = 400) %>% 
  filter(!is.na(price),
         price>1) %>%
  group_by(ticker) %>% 
  mutate(percentage_price = ((price/lag(price,14)) - 1)) %>%
  ungroup() %>% 
  mutate(market_state=case_when(percentage_price>0.02 ~ "+",
                                 percentage_price<(-0.02)~ "-",
                                 TRUE ~ ""))
...
tsla %>% 
   ggplot()+
   geom_rect(data=tsla_states,aes(fill=market_state,xmin=date-14,ymin=ifelse(market_state=="+",0,percentage_price),xmax=date+7,ymax=ifelse(market_state=="+",percentage_price,0)),alpha=0.05)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
  geom_step(aes(date, percentage_price, colour = ticker))+
  ggdark::dark_theme_minimal()+
  geom_hline(yintercept = 0)+
...
eps <- eps %>% 
  mutate(net_earnings=as.numeric(net_earnings))
eps %>% 
  ggplot(aes(id,net_earnings, colour=name))+
  geom_point()+
