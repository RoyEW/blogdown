...
data %>% 
   ggplot()+
   geom_rect(aes(fill=market_state,xmin=date-14,xmax=date+7,ymin=ifelse(market_state=="+",0,data$percentage_price),ymax=ifelse(market_state=="+",data$percentage_price,0)),alpha=0.02)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
  geom_step(aes(date, percentage_price, colour = ticker))+
  geom_hline(yintercept = 0)+
