...
data <- multiplot("TSLA","HYMTF", "FCAU","RACE","NSANY","F","BMW.MI","BMW.DE","HMC","GM","VOW.DE","TM", period = "max",length = 365) %>% 
  filter(!is.na(price),
         price>1) %>%
  group_by(ticker) %>% 
...
data %>% 
   ggplot()+
   geom_rect(data=states,aes(fill=market_state, alpha=market_state,xmin=date,ymin=-Inf,xmax=date+14,ymax=Inf),alpha=0.05)+
  ...
  annotate("text",label="Covid-19",colour="black",x=ymd(20200315),y=0.1,angle=90)+
  theme(axis.text = element_text(),
        legend.position = "none")


tsla <- multiplot("TSLA","HYMTF", "FCAU","RACE","NSANY","F","BMW.MI","BMW.DE","HMC","GM","VOW.DE","TM", period = "max",length = 365) %>% 
  filter(!is.na(price),
         price>1) %>%
  group_by(ticker) %>% 
  mutate(percentage_price = ((price/lag(price,14)) - 1)) %>%
...
tsla %>% 
   ggplot()+
   geom_rect(data=tsla_states,aes(fill=market_state,xmin=date,ymin=-Inf,xmax=date+14,ymax=Inf),alpha=0.05)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
  geom_line(aes(date, percentage_price, colour = ticker))+
  #geom_smooth(aes(date, percentage_price, colour = ticker),se = F)+
  #geom_smooth(colour = "black", lty = 2, aes(date, percentage_price), se = F)+
