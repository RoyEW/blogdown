...
tsla %>% 
  mutate(is_tesla=ifelse(ticker=="TSLA",1,0)) %>% 
   ggplot()+
   geom_rect(data=tsla_states,aes(fill=market_state,xmin=date-0.5,ymin=ifelse(market_state=="+",0,tsla_states$percentage_price),xmax=date+0.5,ymax=ifelse(market_state=="+",tsla_states$percentage_price,0)),alpha=0.2)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
  geom_step(aes(date, percentage_price, colour = ticker))+
  #geom_smooth(aes(date, percentage_price, colour = ticker),se = F)+
  #geom_smooth(colour = "black", lty = 2, aes(date, percentage_price), se = F)+
   theme_void()+
  labs(colour = "Ticker",
       title="")+
  geom_smooth(aes(date,percentage_price,group=is_tesla),colour="grey",lty=2,se=F,method="lm")+
...
data <- multiplot("TSLA","HYMTF", "FCAU","RACE","NSANY","F","BMW.MI","BMW.DE","HMC","GM","VOW.DE","TM", period = "max",length = 400) %>% 
  ...
  ungroup() %>% 
  group_by(date) %>% 
   mutate(market_state=case_when(mean(percentage_price)>0.02 ~ "+",
                                 mean(percentage_price)<(-0.02)~ "-",
                                 TRUE ~ "")) %>% 
   ungroup()
states <- data %>% 
  distinct(date, .keep_all = TRUE)

#fill=market_state,xmin=date-0.5,ymin=ifelse(market_state=="+",0,tsla_states$percentage_price

data %>% 
   ggplot()+
   geom_rect(data=states,aes(fill=market_state,xmin=date-14,ymin=ifelse(market_state=="+",0,percentage_price),xmax=date+7,ymax=ifelse(market_state=="+",percentage_price,0)),alpha=0.04)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
  geom_step(aes(date, percentage_price, colour = ticker))+
  geom_hline(yintercept = 0)+
  ...
  annotate("text",label="Covid-19",colour="black",x=ymd(20200216),y=-0.4,angle=90)+
...
tsla %>% 
   ggplot()+
   geom_rect(data=tsla_states,aes(fill=market_state,xmin=date-14,ymin=ifelse(market_state=="+",0,percentage_price),xmax=date+7,ymax=ifelse(market_state=="+",percentage_price,0)),alpha=0.04)+
  scale_color_manual(values = c("white","red", "green"),aesthetics = c("fill"))+
  geom_step(aes(date, percentage_price, colour = ticker))+
  ggdark::dark_theme_minimal()+
  geom_hline(yintercept = 0)+
