...
dataset
hard.clr.gam <- gam(hardness ~ Al * Al+Co + Cr + Cu + Fe + Ni, data = dataset)




predictions.full <- data.frame(hard_) %>% 
  mutate(prediction.clr.lm = predict(hard.clr.lm, newx = hard.clr),
...
predictions.full %>% 
  ...
  pivot_longer(c(hardness, prediction.alr.lm, prediction.ilr.glm, prediction.clr.gam), names_to = "variable",values_to = "value") %>% 
  ggplot(aes(row_name, value, lty = variable))+
  geom_line()+
