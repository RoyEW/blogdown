...
plot_cases_deaths <- function(c){
  ...
  filter(country == toString(c)) %>%
    ...
    filter(!is.na(value)) %>% 
  ggplot(aes(date, value,colour = rev(name))) +
  geom_line() +
  labs(
    title = paste(c, "Deaths and Cases per 100,000"),
    subtitle = paste("Current daily deaths: ", round(
      covid %>% filter(country == toString(c)) %>% select(deaths) %>% head(1),
      2
    ),". ","Cases per 100,000: ", round(
      covid %>% filter(country == toString(c)) %>% select(cases_rate) %>% head(1))),
    x = "",
    y = "",
    caption = "Source: https://opendata.ecdc.europa.eu/covid19/",
    ...
    geom_smooth(method = "gam",lty = 2, colour = "orange",se=F)+
    geom_point(data = critical_dates %>% filter(country==toString(c)), aes(date,deaths, colour = label),shape=18,size=4)+
  scale_x_date(date_breaks = "1 month", date_minor_breaks = "1 week",
             date_labels = "%B")+
    scale_colour_manual(values = c("orange","red","dodgerblue","green"),aesthetics = c("colour"),labels = c("Cases per 100,000", "Deaths", "Lockdown","Restrictions lifted"))+
    ggthemes::theme_fivethirtyeight()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor.x = element_blank(),
        legend.position = "bottom")#+
    #scale_color_discrete(labels = c("Cases per 100,000", "Deaths", "Lockdown","Restrictions lifted"))
}
