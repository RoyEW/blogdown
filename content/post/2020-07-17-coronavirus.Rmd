...
covid_raw <- read.csv("https://opendata.ecdc.europa.eu/covid19/casedistribution/csv", na.strings = "", fileEncoding = "UTF-8-BOM") %>% 
  rename("date" = dateRep,
         ...
         "cases_rate" = notification_rate_per_100000_population_14.days,
...
covid_g8 %>% 
  ...
  geom_line()+
  labs(title = "Deaths in G8 nations")+
  facet_wrap(~ country)
  
covid_g8 %>% 
  ggplot(aes(date, deaths)) +
  geom_line()+
  labs(title = "Deaths in G20 nations")+
  facet_wrap(~ country)+
...
plot_cases_deaths <- function(c){
  covid %>% 
  filter(country == toString(c)) %>%
    mutate(cases_rate = cases_rate*10) %>%
    mutate(deaths = rollmean(deaths,7,na.pad = T)) %>%
    pivot_longer(cols = c("deaths","cases_rate"), names_to = "name", values_to = "value") %>% 
    filter(!is.na(value)) %>% 
  ggplot(aes(date, value,colour = name)) +
  geom_line() +
  labs(
    title = paste(c, "Deaths and Cases per 10,000"),
    subtitle = paste("Current daily deaths: ", round(
      covid %>% filter(country == toString(c)) %>% select(deaths) %>% head(1),
      2
    ),". ","Cases per 10,000: ", round(
      covid %>% filter(country == toString(c)) %>% select(cases_rate) %>% head(1)),".",sep = ""),
    x = "",
    y = "",
    caption = "Source: https://opendata.ecdc.europa.eu/covid19/",
    colour = ""
    ...
    geom_point(data = critical_dates %>% filter(country==toString(c)), aes(date,cases_rate*10, colour = label),shape=18,size=4)+
  scale_x_date(date_breaks = "1 month", date_minor_breaks = "1 week",
             date_labels = "%B")+
    scale_colour_manual(values = c("orange","red","dodgerblue","green"),aesthetics = c("colour"),labels = c("Cases per 10,000", "Deaths", "Lockdown","Restrictions lifted"))+
    ggthemes::theme_fivethirtyeight()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor.x = element_blank(),
        legend.position = "bottom")#+
...
plot01 <- function(c,...){
  ...
  filter(country == toString(c)) %>%
    mutate(cases_rate = cases_rate*10) %>%
    mutate(deaths = rollmean(deaths,7,na.pad = T)) %>%
    pivot_longer(cols = c(...), names_to = "name", values_to = "value") %>% 
    filter(!is.na(value)) %>% 
  ggplot(aes(date, value,colour = name)) +
  geom_line() +
  labs(
  ...
  scale_x_date(date_breaks = "1 month", date_minor_breaks = "1 week",
             date_labels = "%B")+
    scale_colour_manual(values = c("orange","red","dodgerblue","green","purple"),aesthetics = c("colour"),labels = c("Cases per 10,000", "Deaths", "Lockdown","Restrictions lifted"))+
    ggthemes::theme_fivethirtyeight()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor.x = element_blank(),
        legend.position = "bottom")#+
...
plot_cases_deaths("United Kingdom")
plot_cases_deaths("United States")

plot_cases_deaths("France")
plot_cases_deaths("Germany")
plot_cases_deaths("Pakistan")
plot_cases_deaths("Brazil")
plot_cases_deaths("Algeria")
plot_cases_deaths("Albania")


covid %>% 
  filter(code %in% c(G8,"SWE")) %>% 
  ggplot(aes(date, cases_rate/10)) +
  geom_line()+
  facet_wrap(~ country)
...
covid %>%
  ...
  ggplot(aes(date,value, colour = name)) + 
  geom_line()+
  scale_colour_manual(values = c(deaths = "red",cases_rate = "orange"))+
  facet_wrap(~country)
