...
predictions.full %>% 
  ...
  pivot_longer(c(hardness, pre, prediction.clr.lm,prediction.ilr.glm, prediction.clr.gam), names_to = "variable",values_to = "value") %>% 
...
```{r}
predictions.full <- data.frame(hard) %>% 
  mutate(prediction.clr.lm = predict(hard.clr.lm, newx = hard))

preds <- predict(hard.alr.lm, newdata = hard)
