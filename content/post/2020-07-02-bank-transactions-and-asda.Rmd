...
Cumulative wages and costs specific to ASDA.
...
data %>% 
  arrange(date) %>%   
  filter(str_detect(description, "(Zizza)|(PIZZA)|(SUBWAY)|(BAR)|(MCDONALDS)|(TAKEAWAY)|(GREGGS)"))  %>% 
  mutate(cumsum=cumsum(dr)) %>% 
  ggplot(aes(date,cumsum)) + 
  geom_line() +
  ggtitle("Money spent in Takeaways") +
  scale_y_continuous(labels = scales::dollar_format(prefix = "£"))+
  bbc_style()
view(data)
data %>% 
  arrange(date) %>%   
  filter(str_detect(description, "(ZIZZA)|(TAKEAWAY)"))  %>% 
  mutate(cumsum=cumsum(dr)) %>% 
  ggplot(aes(date,cumsum)) + 
  geom_line() +
  ggtitle("Money spent in Zizza Pizza") +
  scale_y_continuous(labels = scales::dollar_format(prefix = "£"))+
  bbc_style()
```
